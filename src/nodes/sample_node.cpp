...
namespace executor_test
{
    ...
    : Node("sample_node", options)
    {
        counter_called_long_elapsed = 0;
        counter_called_short_elapsed = 0;
        counter_called_subscriber = 0;
        // timer.
        long_timer_ = this->create_wall_timer(1s, std::bind(&SampleNode::count_long_elapsed_time, this));
        short_timer_ = this->create_wall_timer(200ms, std::bind(&SampleNode::count_short_elapsed_time, this));
        // subscriber.
        msg_sub_ = this->create_subscription<std_msgs::msg::String>("message", 1, std::bind(&SampleNode::print_sub_message, this, _1));
        // publisher.
        msg_pub_ = this->create_publisher<std_msgs::msg::String>("message", 10);
    }
...
RCLCPP_COMPONENTS_REGISTER_NODE(executor_test::SampleNode)